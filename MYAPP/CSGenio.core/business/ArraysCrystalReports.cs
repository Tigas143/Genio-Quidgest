...
namespace CSGenio.business
{
    ...
    public class ArraysCrystalReports
    {
        ...
        static ArraysCrystalReports()
		{
			...
			// status
            Qresult = new StringBuilder();
			            Qresult.AppendLine("if {{{0}}} = \"A\" then \"Active\" else");
			            Qresult.AppendLine("if {{{0}}} = \"I\" then \"Inactive\" else");
            Qresult.Append("\"                                              \"");
            todasArrays.Add("status", Qresult.ToString());
			// statusm
            Qresult = new StringBuilder();
			Qresult.AppendLine("if {{{0}}} = 0 then \"Inactive\" else");
			Qresult.AppendLine("if {{{0}}} = 1 then \"Active\" else");
            Qresult.Append("\"                                              \"");
            todasArrays.Add("statusm", Qresult.ToString());
        }
