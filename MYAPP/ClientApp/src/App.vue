<template>
	<q-layout
		...
		:loading="isLoading"
		:loading-menus="!menuLoadMonitor.loaded">
		...
		<template #layout-content>
			...
			<q-cookies
				v-if="this.system.cookies.cookieActive && shouldShowCookies"
				:text="this.Resources[this.system.cookies.cookieText]"
				:file-path="this.system.cookies.filePath"
...
<script>
	...
	export default {
		...
		{
			return {
				buttontext: computed(() => this.Resources[hardcodedTexts.tomeiConhecimento]),
		...
		created()
		{
			this.mainAppLoadMonitor.add(loadResources(this, ['hardcoded', 'messages', 'projectArrays', 'TreeMenu']), true)
		...
		computed: {
			...
			//how or not the cookies
			shouldShowCookies(){
				return this.system.cookies.shouldShowCookies
			},
		...
		methods: {
			...
			/**
			 * get the value of the cookie in the localStorage.
			 */
			checkCookiesState()
			{
			...
			getMenus()
			{
				if (this.system.currentModule === 'Public')
					return
