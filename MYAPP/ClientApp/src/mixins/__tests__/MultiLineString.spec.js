...
import { MultiLineString } from '@/mixins/formModelFieldTypes.js'
...
describe('MultiLineString', () => {
	...
	it.each([null, undefined, ''])('Empty text is correctly displayed when the value is %s', async (emptyValue) => {
	...
	it('Format of a string with no new lines is the same in the server and web', async () => {
	...
	it('Format of a string with new lines, created in a windows application, is sent to the server with \\r\\n', async () => {
		...
		expect(string.value).toStrictEqual(text)
		expect(string.displayValue).toStrictEqual(text)//web text only removes \r when the text is edited
		expect(string.serverValue).toStrictEqual(text)
	})
})
