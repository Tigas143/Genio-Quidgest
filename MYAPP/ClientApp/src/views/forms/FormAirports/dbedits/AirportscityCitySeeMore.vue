<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-airportscity-city-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['AIRPORTSCITY_CITY_____SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'AirportscityCitySeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('AIRPORTSCITY_CITY_____SeeMore', false),

				interfaceMetadata: {
					id: 'AIRPORTSCITY_CITY_____SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'AIRPORTSCITY_CITY_____SeeMore',
					name: 'AIRPORTSCITY_CITY_____SeeMore',
					controller: 'AIRPT',
					action: 'AIRPORTS_CityValCity'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				...
				isActive: true,
				returnElement: 'AIRPORTSCITY_CITY____'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-airportscity-city')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'Airports_Airports_CityValCity',
