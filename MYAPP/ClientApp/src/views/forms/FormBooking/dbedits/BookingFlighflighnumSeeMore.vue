<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-booking-flighflighnum-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['BOOKING_FLIGHFLIGHNUM_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'BookingFlighflighnumSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('BOOKING_FLIGHFLIGHNUM_SeeMore', false),

				interfaceMetadata: {
					id: 'BOOKING_FLIGHFLIGHNUM_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'BOOKING_FLIGHFLIGHNUM_SeeMore',
					name: 'BOOKING_FLIGHFLIGHNUM_SeeMore',
					controller: 'BOOKG',
					action: 'BOOKING_FlighValFlighnum'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-booking-flighflighnum',
				headerTitle: computed(() => this.Resources.FLIGHT55228),
				closeButtonEnable: true,
				hideFooter: true,
				dismissWithEsc: true,
				dismissAction: this.close,
				isActive: true,
				returnElement: 'BOOKING_FLIGHFLIGHNUM'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-booking-flighflighnum')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'Booking_Booking_FlighValFlighnum',
