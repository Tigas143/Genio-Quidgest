<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-flight-crew-crewname-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['FLIGHT__CREW_CREWNAME_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'FlightCrewCrewnameSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('FLIGHT__CREW_CREWNAME_SeeMore', false),

				interfaceMetadata: {
					id: 'FLIGHT__CREW_CREWNAME_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'FLIGHT__CREW_CREWNAME_SeeMore',
					name: 'FLIGHT__CREW_CREWNAME_SeeMore',
					controller: 'FLIGH',
					action: 'FLIGHT_CrewValCrewname'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-flight-crew-crewname',
				...
				isActive: true,
				returnElement: 'FLIGHT__CREW_CREWNAME'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-flight-crew-crewname')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'Flight_Flight_CrewValCrewname',
