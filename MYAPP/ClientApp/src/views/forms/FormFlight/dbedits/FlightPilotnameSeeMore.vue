<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-flight-pilotname-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['FLIGHT__PILOTNAME_____SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'FlightPilotnameSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('FLIGHT__PILOTNAME_____SeeMore', false),

				interfaceMetadata: {
					id: 'FLIGHT__PILOTNAME_____SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'FLIGHT__PILOTNAME_____SeeMore',
					name: 'FLIGHT__PILOTNAME_____SeeMore',
					controller: 'FLIGH',
					action: 'FLIGHT_PilotValName'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-flight-pilotname',
				headerTitle: computed(() => this.Resources.PILOT61493),
				closeButtonEnable: true,
				hideFooter: true,
				dismissWithEsc: true,
				dismissAction: this.close,
				isActive: true,
				returnElement: 'FLIGHT__PILOTNAME____'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-flight-pilotname')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'Flight_Flight_PilotValName',
