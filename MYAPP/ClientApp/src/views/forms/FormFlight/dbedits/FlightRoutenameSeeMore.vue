<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-flight-routename-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['FLIGHT__ROUTENAME_____SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'FlightRoutenameSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('FLIGHT__ROUTENAME_____SeeMore', false),

				interfaceMetadata: {
					id: 'FLIGHT__ROUTENAME_____SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'FLIGHT__ROUTENAME_____SeeMore',
					name: 'FLIGHT__ROUTENAME_____SeeMore',
					controller: 'FLIGH',
					action: 'FLIGHT_RouteValName'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-flight-routename',
				headerTitle: computed(() => this.Resources.ROUTES16793),
				closeButtonEnable: true,
				hideFooter: true,
				dismissWithEsc: true,
				dismissAction: this.close,
				isActive: true,
				returnElement: 'FLIGHT__ROUTENAME____'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-flight-routename')
		},
				...
				const listProps = {
					configuration: {
						...
						config: {
							...
							defaultSearchColumnName: 'Airsc.ValName',
							defaultSearchColumnNameOriginal: 'Airsc.ValName',
							initialSortColumnName: '',
							initialSortColumnOrder: 'asc'
						},
						changeEvents: ['changed-AIRDS', 'changed-AIRSC', 'changed-AIRLN', 'changed-ROUTE'],
						uuid: 'Flight_Flight_RouteValName',
