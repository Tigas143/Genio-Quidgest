<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-planes-aircrname-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['PLANES__AIRCRNAME_____SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'PlanesAircrnameSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('PLANES__AIRCRNAME_____SeeMore', false),

				interfaceMetadata: {
					id: 'PLANES__AIRCRNAME_____SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'PLANES__AIRCRNAME_____SeeMore',
					name: 'PLANES__AIRCRNAME_____SeeMore',
					controller: 'PLANE',
					action: 'PLANES_AircrValName'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-planes-aircrname',
				headerTitle: computed(() => this.Resources.CURRENT_AIRPORTS62631),
				closeButtonEnable: true,
				hideFooter: true,
				dismissWithEsc: true,
				dismissAction: this.close,
				isActive: true,
				returnElement: 'PLANES__AIRCRNAME____'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-planes-aircrname')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						config: {
							name: 'Planes_AircrValName',
							serverMode: true,
							pkColumn: 'ValCodairpt',
							tableAlias: 'AIRCR',
							tableNamePlural: computed(() => this.Resources.CURRENT_AIRPORTS62631),
							viewManagement: '',
							showLimitsInfo: true,
							tableTitle: '',
						...
						changeEvents: ['changed-CITY', 'changed-AIRCR'],
						uuid: 'Planes_Planes_AircrValName',
