<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-routes-airdsname-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['ROUTES__AIRDSNAME_____SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'RoutesAirdsnameSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('ROUTES__AIRDSNAME_____SeeMore', false),

				interfaceMetadata: {
					id: 'ROUTES__AIRDSNAME_____SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'ROUTES__AIRDSNAME_____SeeMore',
					name: 'ROUTES__AIRDSNAME_____SeeMore',
					controller: 'ROUTE',
					action: 'ROUTES_AirdsValName'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-routes-airdsname',
				headerTitle: computed(() => this.Resources.DESTINATION_AIRPORTS61549),
				closeButtonEnable: true,
				hideFooter: true,
				dismissWithEsc: true,
				dismissAction: this.close,
				isActive: true,
				returnElement: 'ROUTES__AIRDSNAME____'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-routes-airdsname')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						config: {
							name: 'Routes_AirdsValName',
							serverMode: true,
							pkColumn: 'ValCodairpt',
							tableAlias: 'AIRDS',
							tableNamePlural: computed(() => this.Resources.DESTINATION_AIRPORTS61549),
							viewManagement: '',
							showLimitsInfo: true,
							tableTitle: '',
						...
						changeEvents: ['changed-AIRDS', 'changed-CITY'],
						uuid: 'Routes_Routes_AirdsValName',
