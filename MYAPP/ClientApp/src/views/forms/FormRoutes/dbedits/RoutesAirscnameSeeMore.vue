<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-routes-airscname-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['ROUTES__AIRSCNAME_____SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'RoutesAirscnameSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('ROUTES__AIRSCNAME_____SeeMore', false),

				interfaceMetadata: {
					id: 'ROUTES__AIRSCNAME_____SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'ROUTES__AIRSCNAME_____SeeMore',
					name: 'ROUTES__AIRSCNAME_____SeeMore',
					controller: 'ROUTE',
					action: 'ROUTES_AirscValName'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-routes-airscname',
				headerTitle: computed(() => this.Resources.SOURCE_AIRPORTS14138),
				closeButtonEnable: true,
				hideFooter: true,
				dismissWithEsc: true,
				dismissAction: this.close,
				isActive: true,
				returnElement: 'ROUTES__AIRSCNAME____'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-routes-airscname')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						config: {
							name: 'Routes_AirscValName',
							serverMode: true,
							pkColumn: 'ValCodairpt',
							tableAlias: 'AIRSC',
							tableNamePlural: computed(() => this.Resources.SOURCE_AIRPORTS14138),
							viewManagement: '',
							showLimitsInfo: true,
							tableTitle: '',
						...
						changeEvents: ['changed-AIRSC', 'changed-CITY'],
						uuid: 'Routes_Routes_AirscValName',
