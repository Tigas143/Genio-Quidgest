...
<script>
	...
	import RegisterButton from '@/views/shared/RegisterButton.vue'
	...
	export default {
		...
		computed: {
			...
			errorMessage()
			{
				if(this.passError) return this.returnMessage["Password"][0]
				else if(this.hasError) return this.returnMessage["Error"][0]
		...
		methods: {
			...
			confirmBox2FA()
			{
				...
				displayMessage(this.Resources[hardcodedTexts.enter6DigitCode], 'question', null, buttons, {
					input: {
						type: 'text',
						placeholder: '000000',
			...
			{
				delete this.returnMessage["Password"] // When the user starts typing hide the error message
