...
namespace GenioMVC.Controllers
{
	...
	{
		private static readonly NavigationLocation ACTION_PJF_MENU_221 = new NavigationLocation("AIRPORTS14051", "PJF_Menu_221", "Airpt") { vueRouteName = "menu-PJF_221" };


		//
		// GET: /Airpt/PJF_Menu_221
		[ActionName("PJF_Menu_221")]
		[HttpPost]
		public ActionResult PJF_Menu_221([FromBody]RequestMenuModel requestModel)
		{
			...
			PJF_Menu_221_ViewModel model = new PJF_Menu_221_ViewModel(UserContext.Current);
			...
			if (isHomePage)
				Navigation.SetValue("HomePage", "PJF_Menu_221");

			//If there was a recent operation on this table then force the primary persistence server to be called and ignore the read only feature
			if (string.IsNullOrEmpty(Navigation.GetStrValue("ForcePrimaryRead_airpt")))
				UserContext.Current.SetPersistenceReadOnly(true);
			else
			{
				Navigation.DestroyEntry("ForcePrimaryRead_airpt");
			...
			if (!isHomePage &&
				(Navigation.CurrentLevel == null || !ACTION_PJF_MENU_221.IsSameAction(Navigation.CurrentLevel.Location)) &&
				Navigation.CurrentLevel.Location.Action != ACTION_PJF_MENU_221.Action)
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.MENU01948 + " " + Navigation.CurrentLevel.Location.ShortDescription());
			else if (isHomePage)
			{
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.MENU01948 + " " + ACTION_PJF_MENU_221.ShortDescription());
				Navigation.SetValue("HomePageContainsList", true);
			}



// USE /[MANUAL PJF MENU_GET 221]/
			...
			if (querystring["ExportList"] != null && Convert.ToBoolean(querystring["ExportList"]) && querystring["ExportType"] != null)
			{
				...
				string file = "PJF_Menu_221_" + DateTime.Now.ToString("ddMMyyyyhhmmss") + "." + exportType;
				ListingMVC<CSGenioAairpt> listing = null;
				...
				byte[] fileBytes = null;
// USE /[MANUAL PJF OVERRQEXPORT 221]/
				fileBytes = new CSGenio.framework.Exports(UserContext.Current.User).ExportList(listing, conditions, columns, exportType, file,ACTION_PJF_MENU_221.Name);
			...
			if (querystring["ImportList"] != null && Convert.ToBoolean(querystring["ImportList"]) && querystring["ImportType"] != null)
			{
				...
				string file = "PJF_Menu_221_Template" + "." + importType;
				List<CSGenio.framework.Exports.QColumn> columns = null;
				model.LoadToExportTemplate(out columns);
				byte[] fileBytes = null;

				fileBytes = new CSGenio.framework.Exports(UserContext.Current.User).ExportTemplate(columns, importType, file,ACTION_PJF_MENU_221.Name);
		...
		//
		// POST: /Airpt/PJF_Menu_221_UploadFile
		[HttpPost]
		public ActionResult PJF_Menu_221_UploadFile(string importType, string qqfile) {
			PJF_Menu_221_ViewModel model = new PJF_Menu_221_ViewModel(UserContext.Current);

			PersistentSupport sp = UserContext.Current.PersistentSupport;
			List<CSGenioAairpt> rows = new List<CSGenioAairpt>();
			...
			try
			{
				...
				rows = new CSGenio.framework.Exports( UserContext.Current.User).ImportList<CSGenioAairpt>(columns, importType, fileBytes);

				sp.openTransaction();
				int lineNumber = 0;
				foreach (CSGenioAairpt importRow in rows)
				{
					try
					{
						lineNumber++;
