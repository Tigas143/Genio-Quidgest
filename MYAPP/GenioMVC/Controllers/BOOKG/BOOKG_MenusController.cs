...
namespace GenioMVC.Controllers
{
	...
	{
		private static readonly NavigationLocation ACTION_PJF_MENU_51 = new NavigationLocation("BOOKING10841", "PJF_Menu_51", "Bookg") { vueRouteName = "menu-PJF_51" };


		//
		// GET: /Bookg/PJF_Menu_51
		[ActionName("PJF_Menu_51")]
		[HttpPost]
		public ActionResult PJF_Menu_51([FromBody]RequestMenuModel requestModel)
		{
			...
			PJF_Menu_51_ViewModel model = new PJF_Menu_51_ViewModel(UserContext.Current);
			...
			if (isHomePage)
				Navigation.SetValue("HomePage", "PJF_Menu_51");

			//If there was a recent operation on this table then force the primary persistence server to be called and ignore the read only feature
			if (string.IsNullOrEmpty(Navigation.GetStrValue("ForcePrimaryRead_bookg")))
				UserContext.Current.SetPersistenceReadOnly(true);
			else
			{
				Navigation.DestroyEntry("ForcePrimaryRead_bookg");
			...
			if (!isHomePage &&
				(Navigation.CurrentLevel == null || !ACTION_PJF_MENU_51.IsSameAction(Navigation.CurrentLevel.Location)) &&
				Navigation.CurrentLevel.Location.Action != ACTION_PJF_MENU_51.Action)
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.MENU01948 + " " + Navigation.CurrentLevel.Location.ShortDescription());
			else if (isHomePage)
			{
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.MENU01948 + " " + ACTION_PJF_MENU_51.ShortDescription());
				Navigation.SetValue("HomePageContainsList", true);
			}



// USE /[MANUAL PJF MENU_GET 51]/
			...
			if (querystring["ExportList"] != null && Convert.ToBoolean(querystring["ExportList"]) && querystring["ExportType"] != null)
			{
				...
				string file = "PJF_Menu_51_" + DateTime.Now.ToString("ddMMyyyyhhmmss") + "." + exportType;
				...
				byte[] fileBytes = null;
// USE /[MANUAL PJF OVERRQEXPORT 51]/
				fileBytes = new CSGenio.framework.Exports(UserContext.Current.User).ExportList(listing, conditions, columns, exportType, file,ACTION_PJF_MENU_51.Name);
			...
			if (querystring["ImportList"] != null && Convert.ToBoolean(querystring["ImportList"]) && querystring["ImportType"] != null)
			{
				...
				string file = "PJF_Menu_51_Template" + "." + importType;
				List<CSGenio.framework.Exports.QColumn> columns = null;
				model.LoadToExportTemplate(out columns);
				byte[] fileBytes = null;

				fileBytes = new CSGenio.framework.Exports(UserContext.Current.User).ExportTemplate(columns, importType, file,ACTION_PJF_MENU_51.Name);
		...
		//
		// POST: /Bookg/PJF_Menu_51_UploadFile
		[HttpPost]
		public ActionResult PJF_Menu_51_UploadFile(string importType, string qqfile) {
			PJF_Menu_51_ViewModel model = new PJF_Menu_51_ViewModel(UserContext.Current);

			PersistentSupport sp = UserContext.Current.PersistentSupport;
			List<CSGenioAbookg> rows = new List<CSGenioAbookg>();
			...
			try
			{
				...
				rows = new CSGenio.framework.Exports( UserContext.Current.User).ImportList<CSGenioAbookg>(columns, importType, fileBytes);

				sp.openTransaction();
				int lineNumber = 0;
				foreach (CSGenioAbookg importRow in rows)
				{
					try
					{
						lineNumber++;
