...
namespace GenioMVC.Controllers
{
	...
	{
		private static readonly NavigationLocation ACTION_PJF_MENU_621 = new NavigationLocation("CABINCREW52945", "PJF_Menu_621", "Crew") { vueRouteName = "menu-PJF_621" };


		//
		// GET: /Crew/PJF_Menu_621
		[ActionName("PJF_Menu_621")]
		[HttpPost]
		public ActionResult PJF_Menu_621([FromBody]RequestMenuModel requestModel)
		{
			...
			PJF_Menu_621_ViewModel model = new PJF_Menu_621_ViewModel(UserContext.Current);
			...
			if (isHomePage)
				Navigation.SetValue("HomePage", "PJF_Menu_621");
			...
			if (!isHomePage &&
				(Navigation.CurrentLevel == null || !ACTION_PJF_MENU_621.IsSameAction(Navigation.CurrentLevel.Location)) &&
				Navigation.CurrentLevel.Location.Action != ACTION_PJF_MENU_621.Action)
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.MENU01948 + " " + Navigation.CurrentLevel.Location.ShortDescription());
			else if (isHomePage)
			{
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.MENU01948 + " " + ACTION_PJF_MENU_621.ShortDescription());
				Navigation.SetValue("HomePageContainsList", true);
			}



// USE /[MANUAL PJF MENU_GET 621]/
			...
			if (querystring["ExportList"] != null && Convert.ToBoolean(querystring["ExportList"]) && querystring["ExportType"] != null)
			{
				...
				string file = "PJF_Menu_621_" + DateTime.Now.ToString("ddMMyyyyhhmmss") + "." + exportType;
				...
				byte[] fileBytes = null;
// USE /[MANUAL PJF OVERRQEXPORT 621]/
				fileBytes = new CSGenio.framework.Exports(UserContext.Current.User).ExportList(listing, conditions, columns, exportType, file,ACTION_PJF_MENU_621.Name);
			...
			if (querystring["ImportList"] != null && Convert.ToBoolean(querystring["ImportList"]) && querystring["ImportType"] != null)
			{
				...
				string file = "PJF_Menu_621_Template" + "." + importType;
				List<CSGenio.framework.Exports.QColumn> columns = null;
				model.LoadToExportTemplate(out columns);
				byte[] fileBytes = null;

				fileBytes = new CSGenio.framework.Exports(UserContext.Current.User).ExportTemplate(columns, importType, file,ACTION_PJF_MENU_621.Name);
		...
		//
		// POST: /Crew/PJF_Menu_621_UploadFile
		[HttpPost]
		public ActionResult PJF_Menu_621_UploadFile(string importType, string qqfile) {
			PJF_Menu_621_ViewModel model = new PJF_Menu_621_ViewModel(UserContext.Current);

			PersistentSupport sp = UserContext.Current.PersistentSupport;
			List<CSGenioAcrew> rows = new List<CSGenioAcrew>();
			...
			try
			{
				...
				rows = new CSGenio.framework.Exports( UserContext.Current.User).ImportList<CSGenioAcrew>(columns, importType, fileBytes);

				sp.openTransaction();
				int lineNumber = 0;
				foreach (CSGenioAcrew importRow in rows)
				{
					try
					{
						lineNumber++;
