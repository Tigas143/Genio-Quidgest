...
namespace GenioMVC.Controllers
{
	...
	public class ControllerBase : ControllerExtension
	{
		...
		[HttpGet]
		public ActionResult GetImage(string id, string modelname, string fldname, string formIdentifier, int height = -1, int width = -1)
		{
			...
			try
			{
				...
				ArgumentException.ThrowIfNullOrEmpty(fldname, nameof(fldname));

				var user = m_userContext.User;
				var area = CSGenio.business.Area.createArea(modelname.ToLowerInvariant(), user, user.CurrentModule)
					?? throw new BusinessException("Incorrect request.", "GetImage", $"The '{modelname}' area does not exist.");

				fldname = fldname.ToLowerInvariant();
				if (!area.DBFields.ContainsKey(fldname))
					throw new BusinessException("Incorrect request.", "GetImage", $"Requested field '{fldname}' does not exist in '{modelname}' area.");
				...
				var condition = CriteriaSet.And()
						.Equal(area.Alias, area.PrimaryKeyName, id);
		...
		protected ActionResult GetFileVersions(string ticket)
		{
			try
			{
				ResourceQuery recq = GetResourceFromTicket(ticket);
		...
		protected ActionResult SetFilesState(List<RequestDocumChangeModel> documents)
		{
			try
			{
				...
				{
					ResourceQuery recq = GetResourceFromTicket(docum.Ticket);
					...
					if (docum.Delete)
					{
						...
						if (external)
						{
							...
							FileUpload file = new(recq.Table, recq.KeyData, fileName);
		...
		protected ActionResult SetFile(string ticket, VersionSubmitAction mode = VersionSubmitAction.Insert, string version = "1")
		{
			try
			{
				...
				if (external)
				{
					...
					FileUpload fileupload = new(recq.Table, recq.KeyData, file.Name);

					// Delete old file if it exists.
					if (fileupload.Delete(oldFile) && fileupload.Save(file.File))
					{
		...
		protected ActionResult GetFile(string ticket, DocumentViewTypeMode viewType = DocumentViewTypeMode.Print)
		{
			try
			{
				...
				if (isFromDocums)
				{
					...
					modelKey = new Guid(documModel.ValModelkey).ToString();
				}
				...
				else
				{
					...
					if (isFromDocums)
						file = GenioMVC.Models.ModelBase.GetSpecificDocument(recq.KeyValue, m_userContext);
					else
				...
				if (viewType == DocumentViewTypeMode.Preview)
				{
					...
					// It must be like this to be possible to open the file in a new tab and preview, if we add "fileName" parameter
					// it will crash with the following error: ERR_RESPONSE_HEADERS_MULTIPLE_CONTENT_DISPOSITION.
		...
		protected ResourceQuery GetResourceFromTicket(string ticket)
		{
			...
			if (username != m_userContext.User.Name || ip != HttpContext.GetIpAddress())
				return null;
			...
			throw new BusinessException(Resources.Resources.OCORREU_UM_ERRO_AO_P53091, "GetResourceFromTicket", "Resource wasn't of type ResourceQuery.");
		}
...
// USE /[MANUAL PJF BEFORE_FILL_EPH]/
