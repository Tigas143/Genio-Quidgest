...
namespace GenioMVC.Controllers
{
	...
	{
		private static readonly NavigationLocation ACTION_PJF_MENU_251 = new NavigationLocation("FLIGHT55228", "PJF_Menu_251", "Fligh") { vueRouteName = "menu-PJF_251" };


		//
		// GET: /Fligh/PJF_Menu_251
		[ActionName("PJF_Menu_251")]
		[HttpPost]
		public ActionResult PJF_Menu_251([FromBody]RequestMenuModel requestModel)
		{
			...
			PJF_Menu_251_ViewModel model = new PJF_Menu_251_ViewModel(UserContext.Current);
			...
			if (isHomePage)
				Navigation.SetValue("HomePage", "PJF_Menu_251");
			...
			if (!isHomePage &&
				(Navigation.CurrentLevel == null || !ACTION_PJF_MENU_251.IsSameAction(Navigation.CurrentLevel.Location)) &&
				Navigation.CurrentLevel.Location.Action != ACTION_PJF_MENU_251.Action)
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.MENU01948 + " " + Navigation.CurrentLevel.Location.ShortDescription());
			else if (isHomePage)
			{
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.MENU01948 + " " + ACTION_PJF_MENU_251.ShortDescription());
				Navigation.SetValue("HomePageContainsList", true);
			}



// USE /[MANUAL PJF MENU_GET 251]/
			...
			if (querystring["ExportList"] != null && Convert.ToBoolean(querystring["ExportList"]) && querystring["ExportType"] != null)
			{
				...
				string file = "PJF_Menu_251_" + DateTime.Now.ToString("ddMMyyyyhhmmss") + "." + exportType;
				...
				byte[] fileBytes = null;
// USE /[MANUAL PJF OVERRQEXPORT 251]/
				fileBytes = new CSGenio.framework.Exports(UserContext.Current.User).ExportList(listing, conditions, columns, exportType, file,ACTION_PJF_MENU_251.Name);
			...
			if (querystring["ImportList"] != null && Convert.ToBoolean(querystring["ImportList"]) && querystring["ImportType"] != null)
			{
				...
				string file = "PJF_Menu_251_Template" + "." + importType;
				List<CSGenio.framework.Exports.QColumn> columns = null;
				model.LoadToExportTemplate(out columns);
				byte[] fileBytes = null;

				fileBytes = new CSGenio.framework.Exports(UserContext.Current.User).ExportTemplate(columns, importType, file,ACTION_PJF_MENU_251.Name);
		...
		//
		// POST: /Fligh/PJF_Menu_251_UploadFile
		[HttpPost]
		public ActionResult PJF_Menu_251_UploadFile(string importType, string qqfile) {
			PJF_Menu_251_ViewModel model = new PJF_Menu_251_ViewModel(UserContext.Current);

			PersistentSupport sp = UserContext.Current.PersistentSupport;
			List<CSGenioAfligh> rows = new List<CSGenioAfligh>();
			...
			try
			{
				...
				rows = new CSGenio.framework.Exports( UserContext.Current.User).ImportList<CSGenioAfligh>(columns, importType, fileBytes);

				sp.openTransaction();
				int lineNumber = 0;
				foreach (CSGenioAfligh importRow in rows)
				{
					try
					{
						lineNumber++;
