...
// USE /[MANUAL PJF HOME_CONTROLLER_INDEX]/
		[HttpGet]
		[AllowAnonymous]
		...
		public ActionResult GetExternalFile([FromQuery]RequestDocumGetModel requestModel)
		{
			try
			{
				ResourceQuery recq = GetResourceFromTicket(requestModel.Ticket);
				PersistentSupport sp = PersistentSupport.getPersistentSupport(UserContext.Current.User.Year);

				byte[] content = recq.GetContent(sp);
				string fileName = recq.Name;
...
// USE /[MANUAL PJF HOME_CONTROLLER_MANUAL]/
