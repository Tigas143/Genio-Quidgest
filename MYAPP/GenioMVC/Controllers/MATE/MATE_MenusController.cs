...
namespace GenioMVC.Controllers
{
	...
	{
		private static readonly NavigationLocation ACTION_PJF_MENU_631 = new NavigationLocation("CREWMATE63640", "PJF_Menu_631", "Mate") { vueRouteName = "menu-PJF_631" };


		//
		// GET: /Mate/PJF_Menu_631
		[ActionName("PJF_Menu_631")]
		[HttpPost]
		public ActionResult PJF_Menu_631([FromBody]RequestMenuModel requestModel)
		{
			...
			PJF_Menu_631_ViewModel model = new PJF_Menu_631_ViewModel(UserContext.Current);
			...
			if (isHomePage)
				Navigation.SetValue("HomePage", "PJF_Menu_631");

			//If there was a recent operation on this table then force the primary persistence server to be called and ignore the read only feature
			if (string.IsNullOrEmpty(Navigation.GetStrValue("ForcePrimaryRead_mate")))
				UserContext.Current.SetPersistenceReadOnly(true);
			else
			{
				Navigation.DestroyEntry("ForcePrimaryRead_mate");
			...
			if (!isHomePage &&
				(Navigation.CurrentLevel == null || !ACTION_PJF_MENU_631.IsSameAction(Navigation.CurrentLevel.Location)) &&
				Navigation.CurrentLevel.Location.Action != ACTION_PJF_MENU_631.Action)
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.MENU01948 + " " + Navigation.CurrentLevel.Location.ShortDescription());
			else if (isHomePage)
			{
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.MENU01948 + " " + ACTION_PJF_MENU_631.ShortDescription());
				Navigation.SetValue("HomePageContainsList", true);
			}



// USE /[MANUAL PJF MENU_GET 631]/
			...
			if (querystring["ExportList"] != null && Convert.ToBoolean(querystring["ExportList"]) && querystring["ExportType"] != null)
			{
				...
				string file = "PJF_Menu_631_" + DateTime.Now.ToString("ddMMyyyyhhmmss") + "." + exportType;
				...
				byte[] fileBytes = null;
// USE /[MANUAL PJF OVERRQEXPORT 631]/
				fileBytes = new CSGenio.framework.Exports(UserContext.Current.User).ExportList(listing, conditions, columns, exportType, file,ACTION_PJF_MENU_631.Name);
			...
			if (querystring["ImportList"] != null && Convert.ToBoolean(querystring["ImportList"]) && querystring["ImportType"] != null)
			{
				...
				string file = "PJF_Menu_631_Template" + "." + importType;
				List<CSGenio.framework.Exports.QColumn> columns = null;
				model.LoadToExportTemplate(out columns);
				byte[] fileBytes = null;

				fileBytes = new CSGenio.framework.Exports(UserContext.Current.User).ExportTemplate(columns, importType, file,ACTION_PJF_MENU_631.Name);
		...
		//
		// POST: /Mate/PJF_Menu_631_UploadFile
		[HttpPost]
		public ActionResult PJF_Menu_631_UploadFile(string importType, string qqfile) {
			PJF_Menu_631_ViewModel model = new PJF_Menu_631_ViewModel(UserContext.Current);

			PersistentSupport sp = UserContext.Current.PersistentSupport;
			List<CSGenioAmate> rows = new List<CSGenioAmate>();
			...
			try
			{
				...
				rows = new CSGenio.framework.Exports( UserContext.Current.User).ImportList<CSGenioAmate>(columns, importType, fileBytes);

				sp.openTransaction();
				int lineNumber = 0;
				foreach (CSGenioAmate importRow in rows)
				{
					try
					{
						lineNumber++;
