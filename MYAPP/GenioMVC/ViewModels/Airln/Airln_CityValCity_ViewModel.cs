...
{
	public class Airln_CityValCity_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "Airln_CityValCity"; }
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="Airln_CityValCity_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Airln_CityValCity_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			{
				// EPH
				crs = Models.City.AddEPH<CSGenioAcity>(ref u, crs, "IBL_AIRLN___CITY_CITY____");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.City.AddEPH<CSGenioAcity>(ref u, null, "IBL_AIRLN___CITY_CITY____"));
			}
...
FieldRef[] fields = new FieldRef[] { CSGenioAcity.FldCodcity, CSGenioAcity.FldZzstate, CSGenioAcity.FldCity };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "IBL_AIRLN___CITY_CITY____");
			...
			conditions.SubSets.Add(airln___city_city____Conds);
			airln___city_city____Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL PJF OVERRQ AIRLN_CITY]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAcity>(m_userContext, false, airln___city_city____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_AIRLN___CITY_CITY____", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL PJF OVERRQLSTEXP AIRLN_CITY]/
			...
			if (tableReload)
			{
// USE /[MANUAL PJF OVERRQLIST AIRLN_CITY]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAcity.GetInformation(), QMVC_POS_RECORD, sorts, airln___city_city____Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAcity> listing = Models.ModelBase.Where<CSGenioAcity>(m_userContext, false, airln___city_city____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_AIRLN___CITY_CITY____", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapAirln_CityValCity(listing);

				Menu.Identifier = "IBL_AIRLN___CITY_CITY____";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_AIRLN___CITY_CITY____";
		...
		private List<Models.City> MapAirln_CityValCity(ListingMVC<CSGenioAcity> Qlisting)
		{
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapAirln_CityValCity(row));
					i++;
				}
			}
		...
		private Models.City MapAirln_CityValCity(CSGenioAcity row)
		{
		...
		#region Custom code
// USE /[MANUAL PJF VIEWMODEL_CUSTOM AIRLN_CITYVALCITY]/
		#endregion
