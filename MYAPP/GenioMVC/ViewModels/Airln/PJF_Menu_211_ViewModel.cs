...
{
	public class PJF_Menu_211_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "bda34664-55a1-4797-abf9-016b81252111"; }
		...
		public override int GetCount(User user)
		{
			...
			var areaBase = CSGenio.business.Area.createArea("airln", user, "PJF");

			//gets eph conditions to be applied in listing
			CriteriaSet conditions = CSGenio.business.Listing.CalculateConditionsEphGeneric(areaBase, "ML211");
			conditions.Equal(CSGenioAairln.FldZzstate, 0); //valid zzstate only
...
FieldRef[] fields = new FieldRef[] { CSGenioAairln.FldCodairln, CSGenioAairln.FldZzstate, CSGenioAairln.FldName, CSGenioAairln.FldCodcity, CSGenioAcity.FldCodcity, CSGenioAcity.FldCity, CSGenioAairln.FldCodairhb, CSGenioAairhb.FldCodairpt, CSGenioAairhb.FldName };

			ListingMVC<CSGenioAairln> listing = new ListingMVC<CSGenioAairln>(fields, null, 1, 1, false, user, true, string.Empty, false);
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="PJF_Menu_211_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public PJF_Menu_211_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			{
				// EPH
				crs = Models.Airln.AddEPH<CSGenioAairln>(ref u, crs, "ML211");
			...
			if (!Navigation.checkFormMode("AIRLN", FormMode.New)) // TODO: Check in Duplicate mode
				crs = extendWithZzstateCondition(crs, CSGenioAairln.FldZzstate, null);


			if (tableReload)
			{
				string QMVC_POS_RECORD = Navigation.GetStrValue("QMVC_POS_RECORD_airln");
				Navigation.DestroyEntry("QMVC_POS_RECORD_airln");
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Airln.AddEPH<CSGenioAairln>(ref u, null, "ML211"));
			}
		...
		public void Load(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAairln> Qlisting, ref CriteriaSet conditions)
		{
			...
			CriteriaSet pjf_menu_211Conds = CriteriaSet.And();
...
FieldRef[] fields = new FieldRef[] { CSGenioAairln.FldCodairln, CSGenioAairln.FldZzstate, CSGenioAairln.FldName, CSGenioAairln.FldCodcity, CSGenioAcity.FldCodcity, CSGenioAcity.FldCity, CSGenioAairln.FldCodairhb, CSGenioAairhb.FldCodairpt, CSGenioAairhb.FldName };
			...
			conditions.SubSets.Add(pjf_menu_211Conds);
			pjf_menu_211Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL PJF OVERRQ 211]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAairln>(m_userContext, false, pjf_menu_211Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML211", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL PJF OVERRQLSTEXP 211]/
...
// USE /[MANUAL PJF OVERRQLIST 211]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAairln.GetInformation(), QMVC_POS_RECORD, sorts, pjf_menu_211Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAairln> listing = Models.ModelBase.Where<CSGenioAairln>(m_userContext, false, pjf_menu_211Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML211", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapPJF_Menu_211(listing);
		...
		private List<Models.Airln> MapPJF_Menu_211(ListingMVC<CSGenioAairln> Qlisting)
		{
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapPJF_Menu_211(row));
					i++;
				}
			}
		...
		private Models.Airln MapPJF_Menu_211(CSGenioAairln row)
		{
			...
			foreach (RequestedField Qfield in row.Fields.Values)
			{
				switch (Qfield.Area)
				{
					...
					case "airhb":
						model.Airhb.klass.insertNameValueField(Qfield.FullName, Qfield.Value); break;
					default:
						break;
				}
			}
		...
		#region Custom code
// USE /[MANUAL PJF VIEWMODEL_CUSTOM PJF_MENU_211]/
		#endregion

		private static readonly string[] _fieldsToSerialize =
		{
			"Airln", "Airln.ValCodairln", "Airln.ValZzstate", "Airln.ValName", "City", "City.ValCity", "Airhb", "Airhb.ValName", "Airln.ValCodairhb", "Airln.ValCodcity"
		};
		...
		private static readonly List<TableSearchColumn> _searchableColumns = new List<TableSearchColumn>
		{
			...
			new TableSearchColumn("Airhb_ValName", CSGenioAairhb.FldName, typeof(string))
		};
	}
}
