...
{
	public class Booking_PersoValName_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "Booking_PersoValName"; }
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="Booking_PersoValName_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Booking_PersoValName_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			{
				// EPH
				crs = Models.Perso.AddEPH<CSGenioAperso>(ref u, crs, "IBL_BOOKING_PERSONAME____");
			...
			crs.Criterias.Add(new Criteria(new ColumnReference(CSGenioAperso.FldZzstate), CriteriaOperator.Equal, 0));


			if (tableReload)
			{
				string QMVC_POS_RECORD = requestValues["Q_POS_RECORD_perso"];
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Perso.AddEPH<CSGenioAperso>(ref u, null, "IBL_BOOKING_PERSONAME____"));
			}
...
FieldRef[] fields = new FieldRef[] { CSGenioAperso.FldCodperso, CSGenioAperso.FldZzstate, CSGenioAperso.FldName };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "IBL_BOOKING_PERSONAME____");
			...
			conditions.SubSets.Add(booking_personame____Conds);
			booking_personame____Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL PJF OVERRQ BOOKING_NAME]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAperso>(m_userContext, false, booking_personame____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_BOOKING_PERSONAME____", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL PJF OVERRQLSTEXP BOOKING_NAME]/
			...
			if (tableReload)
			{
// USE /[MANUAL PJF OVERRQLIST BOOKING_NAME]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAperso.GetInformation(), QMVC_POS_RECORD, sorts, booking_personame____Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAperso> listing = Models.ModelBase.Where<CSGenioAperso>(m_userContext, false, booking_personame____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_BOOKING_PERSONAME____", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapBooking_PersoValName(listing);

				Menu.Identifier = "IBL_BOOKING_PERSONAME____";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_BOOKING_PERSONAME____";
		...
		private List<Models.Perso> MapBooking_PersoValName(ListingMVC<CSGenioAperso> Qlisting)
		{
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapBooking_PersoValName(row));
					i++;
				}
			}
		...
		private Models.Perso MapBooking_PersoValName(CSGenioAperso row)
		{
		...
		#region Custom code
// USE /[MANUAL PJF VIEWMODEL_CUSTOM BOOKING_PERSOVALNAME]/
		#endregion

		private static readonly string[] _fieldsToSerialize =
		{
			"Perso", "Perso.ValCodperso", "Perso.ValZzstate", "Perso.ValName"
		};
