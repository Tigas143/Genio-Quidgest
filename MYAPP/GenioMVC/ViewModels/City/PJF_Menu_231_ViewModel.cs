...
{
	public class PJF_Menu_231_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "51fb9cf9-c169-4b10-9c45-ca53599e5a2c"; }
		...
		public override int GetCount(User user)
		{
			...
			var areaBase = CSGenio.business.Area.createArea("city", user, "PJF");
...
FieldRef[] fields = new FieldRef[] { CSGenioAcity.FldCodcity, CSGenioAcity.FldZzstate, CSGenioAcity.FldCity };
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="PJF_Menu_231_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public PJF_Menu_231_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			{
				// EPH
				crs = Models.City.AddEPH<CSGenioAcity>(ref u, crs, "ML231");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.City.AddEPH<CSGenioAcity>(ref u, null, "ML231"));
			}
		...
		public void Load(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAcity> Qlisting, ref CriteriaSet conditions)
		{
			...
			CriteriaSet pjf_menu_231Conds = CriteriaSet.And();
...
FieldRef[] fields = new FieldRef[] { CSGenioAcity.FldCodcity, CSGenioAcity.FldZzstate, CSGenioAcity.FldCity };
			...
			conditions.SubSets.Add(pjf_menu_231Conds);
			pjf_menu_231Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL PJF OVERRQ 231]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAcity>(m_userContext, false, pjf_menu_231Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML231", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL PJF OVERRQLSTEXP 231]/
...
// USE /[MANUAL PJF OVERRQLIST 231]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAcity.GetInformation(), QMVC_POS_RECORD, sorts, pjf_menu_231Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAcity> listing = Models.ModelBase.Where<CSGenioAcity>(m_userContext, false, pjf_menu_231Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML231", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapPJF_Menu_231(listing);
		...
		private List<Models.City> MapPJF_Menu_231(ListingMVC<CSGenioAcity> Qlisting)
		{
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapPJF_Menu_231(row));
					i++;
				}
			}
		...
		private Models.City MapPJF_Menu_231(CSGenioAcity row)
		{
		...
		#region Custom code
// USE /[MANUAL PJF VIEWMODEL_CUSTOM PJF_MENU_231]/
		#endregion
