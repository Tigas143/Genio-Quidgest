...
{
	public class Crew_AirlnValName_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "Crew_AirlnValName"; }
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="Crew_AirlnValName_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Crew_AirlnValName_ViewModel(UserContext userContext) : base(userContext)
		{
			ValCodcrew = userContext.CurrentNavigation.CurrentLevel.GetEntry("crew")?.ToString();
		}
		...
		public CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			{
				// EPH
				crs = Models.Airln.AddEPH<CSGenioAairln>(ref u, crs, "IBL_CREW____AIRLNNAME____");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Airln.AddEPH<CSGenioAairln>(ref u, null, "IBL_CREW____AIRLNNAME____"));
			}
...
FieldRef[] fields = new FieldRef[] { CSGenioAairln.FldCodairln, CSGenioAairln.FldZzstate };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "IBL_CREW____AIRLNNAME____");
			...
			conditions.SubSets.Add(crew____airlnname____Conds);
			crew____airlnname____Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL PJF OVERRQ CREW_NAME]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAairln>(m_userContext, false, crew____airlnname____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_CREW____AIRLNNAME____", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL PJF OVERRQLSTEXP CREW_NAME]/
			...
			if (tableReload)
			{
// USE /[MANUAL PJF OVERRQLIST CREW_NAME]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAairln.GetInformation(), QMVC_POS_RECORD, sorts, crew____airlnname____Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAairln> listing = Models.ModelBase.Where<CSGenioAairln>(m_userContext, false, crew____airlnname____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_CREW____AIRLNNAME____", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapCrew_AirlnValName(listing);

				Menu.Identifier = "IBL_CREW____AIRLNNAME____";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_CREW____AIRLNNAME____";
		...
		private List<Models.Airln> MapCrew_AirlnValName(ListingMVC<CSGenioAairln> Qlisting)
		{
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapCrew_AirlnValName(row));
					i++;
				}
			}
		...
		private Models.Airln MapCrew_AirlnValName(CSGenioAairln row)
		{
		...
		#region Custom code
// USE /[MANUAL PJF VIEWMODEL_CUSTOM CREW_AIRLNVALNAME]/
		#endregion

		private static readonly string[] _fieldsToSerialize =
		{
			"Airln", "Airln.ValCodairln", "Airln.ValZzstate", "Airln.ValCodairhb", "Airln.ValCodcity"
		};
