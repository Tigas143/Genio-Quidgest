...
{
	public class PJF_Menu_621_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "160bea9d-8961-4724-9f45-eb99e4ea7833"; }
		...
		public override int GetCount(User user)
		{
			...
			var areaBase = CSGenio.business.Area.createArea("crew", user, "PJF");

			//gets eph conditions to be applied in listing
			CriteriaSet conditions = CSGenio.business.Listing.CalculateConditionsEphGeneric(areaBase, "ML621");
			conditions.Equal(CSGenioAcrew.FldZzstate, 0); //valid zzstate only
...
FieldRef[] fields = new FieldRef[] { CSGenioAcrew.FldCodcrew, CSGenioAcrew.FldZzstate, CSGenioAcrew.FldCrewname, CSGenioAcrew.FldCount };

			ListingMVC<CSGenioAcrew> listing = new ListingMVC<CSGenioAcrew>(fields, null, 1, 1, false, user, true, string.Empty, false);
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="PJF_Menu_621_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public PJF_Menu_621_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public override List<Exports.QColumn> GetColumnsToExport(bool ajaxRequest = false)
		{
			var columns = new List<Exports.QColumn>()
			{
				...
				new Exports.QColumn(CSGenioAcrew.FldCount, FieldType.NUMERO, Resources.Resources.NUMBER_OF_CREWMATES54610, 10, 0, true),
			};
		...
		public CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			{
				// EPH
				crs = Models.Crew.AddEPH<CSGenioAcrew>(ref u, crs, "ML621");
			...
			if (!Navigation.checkFormMode("CREW", FormMode.New)) // TODO: Check in Duplicate mode
				crs = extendWithZzstateCondition(crs, CSGenioAcrew.FldZzstate, null);


			if (tableReload)
			{
				string QMVC_POS_RECORD = Navigation.GetStrValue("QMVC_POS_RECORD_crew");
				Navigation.DestroyEntry("QMVC_POS_RECORD_crew");
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Crew.AddEPH<CSGenioAcrew>(ref u, null, "ML621"));
			}
		...
		public void Load(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAcrew> Qlisting, ref CriteriaSet conditions)
		{
			...
			CriteriaSet pjf_menu_621Conds = CriteriaSet.And();
...
FieldRef[] fields = new FieldRef[] { CSGenioAcrew.FldCodcrew, CSGenioAcrew.FldZzstate, CSGenioAcrew.FldCrewname, CSGenioAcrew.FldCount };
			...
			conditions.SubSets.Add(pjf_menu_621Conds);
			pjf_menu_621Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL PJF OVERRQ 621]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAcrew>(m_userContext, false, pjf_menu_621Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML621", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL PJF OVERRQLSTEXP 621]/
...
// USE /[MANUAL PJF OVERRQLIST 621]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAcrew.GetInformation(), QMVC_POS_RECORD, sorts, pjf_menu_621Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAcrew> listing = Models.ModelBase.Where<CSGenioAcrew>(m_userContext, false, pjf_menu_621Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML621", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapPJF_Menu_621(listing);
		...
		private List<Models.Crew> MapPJF_Menu_621(ListingMVC<CSGenioAcrew> Qlisting)
		{
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapPJF_Menu_621(row));
					i++;
				}
			}
		...
		private Models.Crew MapPJF_Menu_621(CSGenioAcrew row)
		{
		...
		#region Custom code
// USE /[MANUAL PJF VIEWMODEL_CUSTOM PJF_MENU_621]/
		#endregion

		private static readonly string[] _fieldsToSerialize =
		{
			"Crew", "Crew.ValCodcrew", "Crew.ValZzstate", "Crew.ValCrewname", "Crew.ValCount", "Crew.ValCodairln"
		};

		private static readonly List<TableSearchColumn> _searchableColumns = new List<TableSearchColumn>
		{
			new TableSearchColumn("ValCrewname", CSGenioAcrew.FldCrewname, typeof(string), defaultSearch : true),
			new TableSearchColumn("ValCount", CSGenioAcrew.FldCount, typeof(decimal?))
		};
	}
}
