...
{
	public class Flight_CrewValCrewname_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "Flight_CrewValCrewname"; }
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="Flight_CrewValCrewname_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Flight_CrewValCrewname_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			{
				// EPH
				crs = Models.Crew.AddEPH<CSGenioAcrew>(ref u, crs, "IBL_FLIGHT__CREW_CREWNAME");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Crew.AddEPH<CSGenioAcrew>(ref u, null, "IBL_FLIGHT__CREW_CREWNAME"));
			}
...
FieldRef[] fields = new FieldRef[] { CSGenioAcrew.FldCodcrew, CSGenioAcrew.FldZzstate, CSGenioAcrew.FldCrewname };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "IBL_FLIGHT__CREW_CREWNAME");
			...
			conditions.SubSets.Add(flight__crew_crewnameConds);
			flight__crew_crewnameConds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL PJF OVERRQ FLIGHT_CREWNAME]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAcrew>(m_userContext, false, flight__crew_crewnameConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_FLIGHT__CREW_CREWNAME", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL PJF OVERRQLSTEXP FLIGHT_CREWNAME]/
			...
			if (tableReload)
			{
// USE /[MANUAL PJF OVERRQLIST FLIGHT_CREWNAME]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAcrew.GetInformation(), QMVC_POS_RECORD, sorts, flight__crew_crewnameConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAcrew> listing = Models.ModelBase.Where<CSGenioAcrew>(m_userContext, false, flight__crew_crewnameConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_FLIGHT__CREW_CREWNAME", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapFlight_CrewValCrewname(listing);

				Menu.Identifier = "IBL_FLIGHT__CREW_CREWNAME";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_FLIGHT__CREW_CREWNAME";
		...
		private List<Models.Crew> MapFlight_CrewValCrewname(ListingMVC<CSGenioAcrew> Qlisting)
		{
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapFlight_CrewValCrewname(row));
					i++;
				}
			}
		...
		private Models.Crew MapFlight_CrewValCrewname(CSGenioAcrew row)
		{
		...
		#region Custom code
// USE /[MANUAL PJF VIEWMODEL_CUSTOM FLIGHT_CREWVALCREWNAME]/
		#endregion
