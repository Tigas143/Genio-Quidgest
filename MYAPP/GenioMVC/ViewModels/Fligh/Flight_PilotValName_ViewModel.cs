...
{
	public class Flight_PilotValName_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "Flight_PilotValName"; }
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="Flight_PilotValName_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Flight_PilotValName_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			{
				// EPH
				crs = Models.Pilot.AddEPH<CSGenioApilot>(ref u, crs, "IBL_FLIGHT__PILOTNAME____");
			...
			crs.Criterias.Add(new Criteria(new ColumnReference(CSGenioApilot.FldZzstate), CriteriaOperator.Equal, 0));


			if (tableReload)
			{
				string QMVC_POS_RECORD = requestValues["Q_POS_RECORD_pilot"];
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Pilot.AddEPH<CSGenioApilot>(ref u, null, "IBL_FLIGHT__PILOTNAME____"));
			}
...
FieldRef[] fields = new FieldRef[] { CSGenioApilot.FldCodpilot, CSGenioApilot.FldZzstate, CSGenioApilot.FldName };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "IBL_FLIGHT__PILOTNAME____");
			...
			conditions.SubSets.Add(flight__pilotname____Conds);
			flight__pilotname____Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL PJF OVERRQ FLIGHT_NAME]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioApilot>(m_userContext, false, flight__pilotname____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_FLIGHT__PILOTNAME____", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL PJF OVERRQLIST FLIGHT_NAME]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioApilot.GetInformation(), QMVC_POS_RECORD, sorts, flight__pilotname____Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioApilot> listing = Models.ModelBase.Where<CSGenioApilot>(m_userContext, false, flight__pilotname____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_FLIGHT__PILOTNAME____", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapFlight_PilotValName(listing);

				Menu.Identifier = "IBL_FLIGHT__PILOTNAME____";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_FLIGHT__PILOTNAME____";
		...
		private List<Models.Pilot> MapFlight_PilotValName(ListingMVC<CSGenioApilot> Qlisting)
		{
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapFlight_PilotValName(row));
					i++;
				}
			}
		...
		private Models.Pilot MapFlight_PilotValName(CSGenioApilot row)
		{
		...
		#region Custom code
// USE /[MANUAL PJF VIEWMODEL_CUSTOM FLIGHT_PILOTVALNAME]/
		#endregion

		private static readonly string[] _fieldsToSerialize =
		{
			"Pilot", "Pilot.ValCodpilot", "Pilot.ValZzstate", "Pilot.ValName", "Pilot.ValCodairln"
		};
