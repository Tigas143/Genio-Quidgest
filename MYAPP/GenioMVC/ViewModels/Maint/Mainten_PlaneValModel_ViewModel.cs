...
{
	public class Mainten_PlaneValModel_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "Mainten_PlaneValModel"; }
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="Mainten_PlaneValModel_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Mainten_PlaneValModel_ViewModel(UserContext userContext) : base(userContext)
		{
			ValCodmaint = userContext.CurrentNavigation.CurrentLevel.GetEntry("maint")?.ToString();
		}
		...
		public override List<Exports.QColumn> GetColumnsToExport(bool ajaxRequest = false)
		{
			...
			{
				new Exports.QColumn(CSGenioAplane.FldModel, FieldType.TEXTO, Resources.Resources.MODEL27263, 10, 0, true),
			};
		...
		public CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			allSortOrders.Add("PLANE.MODEL", new OrderedDictionary());
			allSortOrders["PLANE.MODEL"].Add("PLANE.MODEL", "A");
			...
			{
				// EPH
				crs = Models.Plane.AddEPH<CSGenioAplane>(ref u, crs, "IBL_MAINTEN_PLANEMODEL___");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Plane.AddEPH<CSGenioAplane>(ref u, null, "IBL_MAINTEN_PLANEMODEL___"));
			}
...
FieldRef[] fields = new FieldRef[] { CSGenioAplane.FldCodplane, CSGenioAplane.FldZzstate, CSGenioAplane.FldModel };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "IBL_MAINTEN_PLANEMODEL___");
			...
			conditions.SubSets.Add(mainten_planemodel___Conds);
			mainten_planemodel___Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL PJF OVERRQ MAINTEN_MODEL]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAplane>(m_userContext, false, mainten_planemodel___Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_MAINTEN_PLANEMODEL___", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL PJF OVERRQLSTEXP MAINTEN_MODEL]/
			...
			if (tableReload)
			{
// USE /[MANUAL PJF OVERRQLIST MAINTEN_MODEL]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAplane.GetInformation(), QMVC_POS_RECORD, sorts, mainten_planemodel___Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAplane> listing = Models.ModelBase.Where<CSGenioAplane>(m_userContext, false, mainten_planemodel___Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_MAINTEN_PLANEMODEL___", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapMainten_PlaneValModel(listing);

				Menu.Identifier = "IBL_MAINTEN_PLANEMODEL___";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_MAINTEN_PLANEMODEL___";
		...
		private List<Models.Plane> MapMainten_PlaneValModel(ListingMVC<CSGenioAplane> Qlisting)
		{
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapMainten_PlaneValModel(row));
					i++;
				}
			}
		...
		private Models.Plane MapMainten_PlaneValModel(CSGenioAplane row)
		{
		...
		#region Custom code
// USE /[MANUAL PJF VIEWMODEL_CUSTOM MAINTEN_PLANEVALMODEL]/
		#endregion
