...
{
	public class Pilot_AirlnValName_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "Pilot_AirlnValName"; }
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="Pilot_AirlnValName_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Pilot_AirlnValName_ViewModel(UserContext userContext) : base(userContext)
		{
			ValCodpilot = userContext.CurrentNavigation.CurrentLevel.GetEntry("pilot")?.ToString();
		}
		...
		public CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			{
				// EPH
				crs = Models.Airln.AddEPH<CSGenioAairln>(ref u, crs, "IBL_PILOT___AIRLNNAME____");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Airln.AddEPH<CSGenioAairln>(ref u, null, "IBL_PILOT___AIRLNNAME____"));
			}
...
FieldRef[] fields = new FieldRef[] { CSGenioAairln.FldCodairln, CSGenioAairln.FldZzstate, CSGenioAairln.FldName };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "IBL_PILOT___AIRLNNAME____");
			...
			conditions.SubSets.Add(pilot___airlnname____Conds);
			pilot___airlnname____Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL PJF OVERRQ PILOT_NAME]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAairln>(m_userContext, false, pilot___airlnname____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PILOT___AIRLNNAME____", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL PJF OVERRQLSTEXP PILOT_NAME]/
			...
			if (tableReload)
			{
// USE /[MANUAL PJF OVERRQLIST PILOT_NAME]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAairln.GetInformation(), QMVC_POS_RECORD, sorts, pilot___airlnname____Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAairln> listing = Models.ModelBase.Where<CSGenioAairln>(m_userContext, false, pilot___airlnname____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PILOT___AIRLNNAME____", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapPilot_AirlnValName(listing);

				Menu.Identifier = "IBL_PILOT___AIRLNNAME____";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_PILOT___AIRLNNAME____";
		...
		private List<Models.Airln> MapPilot_AirlnValName(ListingMVC<CSGenioAairln> Qlisting)
		{
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapPilot_AirlnValName(row));
					i++;
				}
			}
		...
		private Models.Airln MapPilot_AirlnValName(CSGenioAairln row)
		{
		...
		#region Custom code
// USE /[MANUAL PJF VIEWMODEL_CUSTOM PILOT_AIRLNVALNAME]/
		#endregion
