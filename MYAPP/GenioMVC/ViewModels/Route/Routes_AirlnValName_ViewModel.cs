...
{
	public class Routes_AirlnValName_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "Routes_AirlnValName"; }
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="Routes_AirlnValName_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Routes_AirlnValName_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			{
				// EPH
				crs = Models.Airln.AddEPH<CSGenioAairln>(ref u, crs, "IBL_ROUTES__AIRLNNAME____");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Airln.AddEPH<CSGenioAairln>(ref u, null, "IBL_ROUTES__AIRLNNAME____"));
			}
...
FieldRef[] fields = new FieldRef[] { CSGenioAairln.FldCodairln, CSGenioAairln.FldZzstate, CSGenioAairln.FldName };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "IBL_ROUTES__AIRLNNAME____");
			...
			conditions.SubSets.Add(routes__airlnname____Conds);
			routes__airlnname____Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL PJF OVERRQ ROUTES_NAME]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAairln>(m_userContext, false, routes__airlnname____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_ROUTES__AIRLNNAME____", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL PJF OVERRQLIST ROUTES_NAME]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAairln.GetInformation(), QMVC_POS_RECORD, sorts, routes__airlnname____Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAairln> listing = Models.ModelBase.Where<CSGenioAairln>(m_userContext, false, routes__airlnname____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_ROUTES__AIRLNNAME____", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapRoutes_AirlnValName(listing);

				Menu.Identifier = "IBL_ROUTES__AIRLNNAME____";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_ROUTES__AIRLNNAME____";
		...
		private List<Models.Airln> MapRoutes_AirlnValName(ListingMVC<CSGenioAairln> Qlisting)
		{
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapRoutes_AirlnValName(row));
					i++;
				}
			}
		...
		private Models.Airln MapRoutes_AirlnValName(CSGenioAairln row)
		{
		...
		#region Custom code
// USE /[MANUAL PJF VIEWMODEL_CUSTOM ROUTES_AIRLNVALNAME]/
		#endregion
