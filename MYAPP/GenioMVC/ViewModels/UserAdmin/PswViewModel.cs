...
namespace GenioMVC.ViewModels.Psw
{
	public class PswViewModel : FormViewModel<Models.Psw>
	{
		...
		private static List<string> modulos = new List<string>() { "PJF" };

		public IDictionary<decimal, string> PjfLevels { get; private set; }
		public decimal? PJFLevel { get; set; }
		public SelectList UserPJF { get; set; }
		...
		public List<GenioMVC.Models.S_ua> FillAuthorizationList()
		{
			...
			cond.Equal(CSGenioAs_ua.FldSistema, "PJF");
			...
			foreach (var m in naoExistentes)
			{
				...
				try
				{
					...
					new_rowuser.klass.ValSistema = "PJF";
		...
		public override void LoadPartial(NameValueCollection qs, bool ajaxRequest = false)
		{
			...
			PjfLevels = new Dictionary<decimal, string>();
			PjfLevels.Add(LevelAccess.DESAUTORIZADO.LevelValue, Resources.Resources.DESAUTORIZADO34584);
			PjfLevels.Add(LevelAccess.NV10.LevelValue, Resources.Resources.STAFF42088);
			PjfLevels.Add(LevelAccess.NV90.LevelValue, Resources.Resources.AIRLINE57868);
			PjfLevels.Add(LevelAccess.NV99.LevelValue, Resources.Resources.ADMINISTRADOR57294);

			selectedAuth = this.AuthorizationList.FirstOrDefault(x => x.ValModulo == "PJF");
			PJFLevel = selectedAuth == null ? LevelAccess.DESAUTORIZADO.LevelValue : selectedAuth.ValNivel;
			this.UserPJF = new SelectList(PjfLevels, "Key", "Value", PJFLevel);
		}
		...
		public void SaveAuthorization()
		{
			...
			Valcodua = AuthorizationList.Find(m => m.ValModulo == "PJF").ValCodua;
			userauth = CSGenio.business.CSGenioAs_ua.search(sp, Valcodua, u);
			userauth.ValDatamuda = DateTime.Now;
			userauth.ValModulo = "PJF";
			userauth.ValNivel = Convert.ToDecimal(PJFLevel);
			userauth.ValRole = PJFLevel.ToString();
