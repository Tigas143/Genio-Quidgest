...

{
    public class UserFactory
    {
        ...
        {
            new Tuple<string,Role>("PJF", Role.UNAUTHORIZED),

			new Tuple<string,Role>("PJF", Role.ROLE_10),
			new Tuple<string,Role>("PJF", Role.ROLE_90),
			new Tuple<string,Role>("PJF", Role.ADMINISTRATION)        };
		...
		public static void MailSender(CSGenioApsw user, string ConfirmURL, string lang = "")
        {
            ...
            else
            {
                try
                {
                    ...
                    // USE /[MANUAL PJF CUSTOM_USER_REGISTRATION]/
