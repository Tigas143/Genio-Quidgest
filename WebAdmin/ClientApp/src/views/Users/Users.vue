      ...
      <div class="tab-pane c-tab__item-content active" id="all-users" ref="all-users" role="tabpanel" aria-labelledby="all-users">
        <row>
          ...
          <qtable :rows="Users.rows"
            ...
            <template #PJF="props">
              <template v-for="userRole in props.row.UserRoles" :key="userRole.Role">
                    <span v-if="userRole.Module === 'PJF'" class="role-tag">{{Resources[userRole.Designation]}}</span>
              </template>
            </template>
  ...
  export default {
    ...
    data: function () {
      ...
      return {
        Model: {},
        Modules: {
          PJF : { active: true, Cod: "PJF", Description: 'AIRLINES_MANAGER26919'},
        },
        Users: {
          rows: [],
          total_rows: 0,
    ...
    computed: {
            selectModules: {
                ...
                get: function () {
                  ...
                  return                     this.Modules.PJF.active ;
                },
                set: function (value) {
                    //If we explicitly declare the variables, vue will bind them
                    this.Modules.PJF.active = value;
                    return value;
                }
            }
    ...
    methods: {
      ...
      userColumns: function () {
        ...
        return [{
          ...
          {
              label: () => vm.$t(vm.Modules.PJF.Description),
              name: "PJF",
              slot_name: "PJF",
              sort: false,
              visibility: vm.Modules.PJF.active && vm.columnModules
          },
          ];
      },
